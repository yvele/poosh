...
export default function mutateJoiSchema(schema: Object, Joi: Object) {
  ...
  const eachSchema = Joi.reach(schema, "each")._inner.items[0];

  const headersSchema = Joi.reach(eachSchema, "headers");

  // eslint-disable-next-line no-underscore-dangle
  const locationChild = headersSchema._inner.children
